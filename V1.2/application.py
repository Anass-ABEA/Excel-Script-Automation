...
def findElementIndex(string, list):
...
def initaliserPageSujets(page):
    cell_format = workbook.add_format()
    cell_format.set_bold()
    cell_format.set_align("center")
    cell_format.set_align("vcenter")

    page.set_column(0, 19, 30)

    page.merge_range("B1:T1", "merged")
    page.merge_range("A1:A2", "merged")

    ligne = 0
    page.write(ligne, 0, "entreprise", cell_format)
    page.write(ligne, 1, "Nature du sujet", cell_format)
    ligne = 1
    x = 1
    for e in listeTypeSujets:
        page.write(ligne, x, e, cell_format)
        x+=1
...
def addToSujets(page,data,ligne):
    cell_format = workbook.add_format()
    cell_format.set_align("center")
    cell_format.set_align("vcenter")
    try:
...
page_sujets = workbook.add_worksheet("sujets")
...
initaliserPageSujets(page_sujets)
...
try:
    for fichier in excel_list:
        ...
        if data["entreprise"] not in listeEntreprises:
            addToSujets(page_sujets,data,ligne_S)
            ligne_S+=1
            listeEntreprises.append(data["entreprise"])
        else:
            addToSujets(page_sujets,data,findElementIndex(data["entreprise"],listeEntreprises)+2)
